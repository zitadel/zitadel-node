...
export class BetaUserServiceIDPSAMLAccessInformation {
  /**
  ...
  constructor(data?: Partial<BetaUserServiceIDPSAMLAccessInformation>) {
     ...
     */
    if (this.assertion != null && typeof this.assertion === "string") {
      this.assertion = Buffer.from(
        this.assertion as unknown as string,
        "base64",
      ) as unknown as Buffer;
    } else if (this.assertion != null && !Buffer.isBuffer(this.assertion)) {
      throw new TypeError(
        `assertion must be a Buffer or base64 string, got ${typeof this.assertion}`,
      );
    }
  }
  ...
  equals(other: unknown): boolean {
    ...
    if (!(other instanceof BetaUserServiceIDPSAMLAccessInformation)) {
      return false;
    }
    if (
      !BetaUserServiceIDPSAMLAccessInformation.__deepEquals(
        this.assertion,
        (other as BetaUserServiceIDPSAMLAccessInformation).assertion,
      )
    ) {
      return false;
    }
  ...
  private static __deepEquals(a: unknown, b: unknown): boolean {
    ...
    if (Array.isArray(a) && Array.isArray(b)) {
      ...
      return a.every((v, i) =>
        BetaUserServiceIDPSAMLAccessInformation.__deepEquals(v, b[i]),
      );
    }
    if (a instanceof Set && b instanceof Set) {
      ...
      return [...a].every((v) =>
        bvals.some((w) =>
          BetaUserServiceIDPSAMLAccessInformation.__deepEquals(v, w),
        ),
      );
    }
    if (a instanceof Map && b instanceof Map) {
      ...
      for (const [k, v] of a) {
        if (
          !b.has(k) ||
          !BetaUserServiceIDPSAMLAccessInformation.__deepEquals(v, b.get(k))
        ) {
          return false;
        }
      }
    ...
    if (typeof a === "object" && typeof b === "object") {
      ...
      return ak.every(
        (k) =>
          ...
          BetaUserServiceIDPSAMLAccessInformation.__deepEquals(ar[k], br[k]),
      );
    }
    return false;
  }
