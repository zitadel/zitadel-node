...
export class ActionServiceListExecutionFunctionsResponse {
  /**
  ...
  constructor(data?: Partial<ActionServiceListExecutionFunctionsResponse>) {
    Object.assign(this, data);
    if (
      this.functions != null &&
      !Array.isArray(this.functions) &&
      !((this.functions as unknown) instanceof Set)
    ) {
      throw new TypeError(
        `functions must be an array, got ${typeof this.functions}`,
      );
    }
  }
  ...
  equals(other: unknown): boolean {
    ...
    if (!(other instanceof ActionServiceListExecutionFunctionsResponse)) {
      return false;
    }
    if (
      !ActionServiceListExecutionFunctionsResponse.__deepEquals(
        this.functions,
        (other as ActionServiceListExecutionFunctionsResponse).functions,
      )
    ) {
      return false;
    }
  ...
  private static __deepEquals(a: unknown, b: unknown): boolean {
    ...
    if (Array.isArray(a) && Array.isArray(b)) {
      ...
      return a.every((v, i) =>
        ActionServiceListExecutionFunctionsResponse.__deepEquals(v, b[i]),
      );
    }
    if (a instanceof Set && b instanceof Set) {
      ...
      return [...a].every((v) =>
        bvals.some((w) =>
          ActionServiceListExecutionFunctionsResponse.__deepEquals(v, w),
        ),
      );
    }
    if (a instanceof Map && b instanceof Map) {
      ...
      for (const [k, v] of a) {
        if (
          !b.has(k) ||
          !ActionServiceListExecutionFunctionsResponse.__deepEquals(v, b.get(k))
        ) {
          return false;
        }
      }
