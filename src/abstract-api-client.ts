// Zitadel SDK
// The Zitadel SDK is a convenience wrapper around the Zitadel APIs to assist you in integrating with your Zitadel environment. This SDK enables you to handle resources, settings, and configurations within the Zitadel platform.
//
// The version of the OpenAPI document: 1.0.0
//
// Generated by: https://openapi-generator.tech

import type { ApiClient, SendRequestOptions } from "./api-client.js";
/* Through the errors barrel, which loads every status-specific error, so
 * ApiError.fromResponse knows them all. */
import { ApiError } from "./errors/index.js";
import { NetworkError } from "./errors/network-error.js";
import { NetworkTimeoutError } from "./errors/network-timeout-error.js";
import type { ApiHttpResponse } from "./api-http-response.js";
import { ObjectSerializer } from "./object-serializer.js";
import { TransportOptions } from "./transport-options.js";

/**
 * Header names that MUST be stripped when following a cross-origin
 * redirect. Includes the classic credential carriers (`Authorization`,
 * `Cookie`, `Proxy-Authorization`) plus the apiKey-in-header header
 * names declared by this spec's security schemes (merged at codegen
 * time from the spec's apiKey-in-header schemes). Leaving an API-key
 * header attached would leak the key to attacker-controlled hosts via
 * a malicious 302/307 from an upstream that supports both bearer and
 * api-key auth. Deriving the set from the spec keeps the
 * credential-stripping boundary accurate and uniform across all SDKs.
 * Match is case-insensitive; entries here MUST be lowercase.
 */
export const SENSITIVE_HEADER_NAMES: ReadonlySet<string> = new Set<string>([
  "authorization",
  "cookie",
  "proxy-authorization",
]);

/**
 * Transport-agnostic base for {@link ApiClient} implementations.
 *
 * Owns every piece of HTTP behaviour that must be identical regardless of the
 * runtime: transport-level default-header merging, User-Agent and X-Request-ID
 * injection, Accept-Encoding negotiation, multipart encoding, the manual
 * redirect loop (with cross-origin sensitive-header stripping, HTTPS->HTTP
 * body-replay refusal, and non-http(s) scheme rejection), response
 * decompression, and charset decoding -- all expressed with Web-standard APIs
 * (`fetch`, `crypto`, `DecompressionStream`, `TextDecoder`). Concrete
 * subclasses supply only the optional dispatcher via {@link prepareDispatcher},
 * so this security-critical logic lives in exactly one place across both the
 * Node transport (with undici proxy/TLS support) and the portable Fetch
 * transport used on Deno, Bun, and edge runtimes.
 *
 * Header merge order (lowest to highest priority):
 * 1. {@link TransportOptions.defaultHeaders} -- transport-level defaults
 * 2. Caller-provided headers (from BaseApi -- includes config defaults, auth, operation headers)
 * 3. {@link TransportOptions.userAgent} -- injected if not already set
 * 4. {@link TransportOptions.injectRequestId} -- injected if not already set
 */
export abstract class AbstractApiClient implements ApiClient {
  protected readonly transportOptions: TransportOptions;
  /* The dispatcher (if any) is resolved once per client and cached. A `null`
   * resolution means "no special dispatcher; use plain fetch". */
  private dispatcherPromise?: Promise<unknown>;
  /* T-D4: once close() has run the client must refuse further use with the
   * SDK's own error type rather than leaking a foreign exception or silently
   * succeeding against a cleared pool. */
  protected closed = false;

  /**
   * Create a client configured from the given {@link TransportOptions}.
   *
   * @param transportOptions transport configuration to apply
   */
  constructor(transportOptions?: TransportOptions) {
    this.transportOptions =
      transportOptions ?? TransportOptions.builder().build();
  }

  /**
   * Resolve the transport dispatcher (if any) to attach to every fetch on this
   * client. The Node transport returns an undici dispatcher carrying proxy /
   * TLS / CA configuration; the portable Fetch transport returns `undefined`
   * for plain requests and throws a typed {@link ApiError} when a Node-only
   * transport feature is requested on a runtime that cannot honour it.
   *
   * @returns a dispatcher to set on `fetch` options, or `undefined` for plain fetch
   */
  protected abstract prepareDispatcher(): Promise<unknown>;

  /**
   * The list of content encodings advertised in `Accept-Encoding`. Overridable
   * so the Node transport can add `zstd` where `node:zlib` supports it; the
   * portable default is the set every fetch implementation decodes natively.
   *
   * @returns a comma-separated `Accept-Encoding` value
   */
  protected supportedEncodings(): string {
    return "gzip, deflate, br";
  }

  /**
   * Classify a failure raised by `fetch` or by the response body read.
   *
   * The `AbortSignal.timeout` signal rejects with a `DOMException` named
   * `TimeoutError`, and undici reports its own connect / headers / body
   * timeouts as a `TypeError` whose cause carries a `UND_ERR_*_TIMEOUT` code:
   * both are a {@link NetworkTimeoutError}. Any other `TypeError` means no
   * HTTP response arrived (connection refused, DNS failure, TLS failure,
   * connection reset) and is a {@link NetworkError}, except a proxy that
   * refused the tunnel: that proxy answered, so the error is the one
   * {@link ApiError.fromResponse} maps its status to (a 407 without proxy
   * credentials is a `ClientError`). Anything else stays a plain
   * {@link ApiError} with status 0. The original error is kept as `cause` in
   * every other case.
   *
   * @param error the value `fetch` or the body read rejected with
   * @returns the SDK error to throw
   */
  protected static transportError(error: unknown): ApiError {
    /* Duck-typed rather than `instanceof`: fetch may reject with an error
     * from another realm (a test sandbox or a polyfill), whose `TypeError`
     * is not this realm's `TypeError`. */
    const field = (value: unknown, key: string): unknown =>
      value != null && typeof value === "object" && key in value
        ? (value as Record<string, unknown>)[key]
        : undefined;
    const name = String(field(error, "name") ?? "");
    const message = String(field(error, "message") ?? error);
    const causeCode = String(field(field(error, "cause"), "code") ?? "");
    if (
      name === "TimeoutError" ||
      /^UND_ERR_(CONNECT|HEADERS|BODY)_TIMEOUT$/.test(causeCode)
    ) {
      return new NetworkTimeoutError(message, { cause: error });
    }
    const proxyStatus = AbstractApiClient.proxyRefusalStatus(error);
    if (proxyStatus !== null) {
      return ApiError.fromResponse(proxyStatus, null, null);
    }
    if (name === "TypeError") {
      return new NetworkError(message, { cause: error });
    }
    return new ApiError(0, message, null, null, null, { cause: error });
  }

  /**
   * The status a proxy refused the CONNECT tunnel with, found anywhere in the
   * error's cause chain (undici reports it only in the message), or null.
   *
   * @param error the value `fetch` rejected with
   * @returns the proxy's status code, or null when no proxy refused
   */
  private static proxyRefusalStatus(error: unknown): number | null {
    let current: unknown = error;
    for (
      let depth = 0;
      depth < 4 && current != null && typeof current === "object";
      depth++
    ) {
      const match = /Proxy response \((\d{3})\) !== 200/.exec(
        String((current as Record<string, unknown>).message ?? ""),
      );
      if (match) {
        return Number(match[1]);
      }
      current = (current as Record<string, unknown>).cause;
    }
    return null;
  }

  /**
   * Whether a body-read failure is the fetch layer failing to decompress
   * the body (a zlib, brotli or zstd error somewhere in the cause chain)
   * rather than the connection failing mid-body.
   *
   * @param error the value the body read rejected with
   * @returns true when the body arrived but could not be decoded
   */
  protected static isDecodeFailure(error: unknown): boolean {
    let current: unknown = error;
    for (
      let depth = 0;
      depth < 4 && current != null && typeof current === "object";
      depth++
    ) {
      const code = String((current as Record<string, unknown>).code ?? "");
      if (/^Z_|BROTLI|ZSTD/.test(code)) {
        return true;
      }
      current = (current as Record<string, unknown>).cause;
    }
    return false;
  }

  /**
   * Resolve and cache the dispatcher for this client, so {@link prepareDispatcher}
   * (and any CA read it performs) runs at most once.
   *
   * @returns the cached dispatcher, or `undefined` for plain fetch
   */
  private getDispatcher(): Promise<unknown> {
    this.dispatcherPromise ??= this.prepareDispatcher();
    return this.dispatcherPromise;
  }

  /**
   * Send an HTTP request and return the response.
   *
   * Merges transport-level default headers, applies User-Agent and
   * X-Request-ID injection, follows redirects (stripping sensitive headers on
   * cross-origin hops and refusing unsafe replays), and decompresses and
   * charset-decodes the response body.
   *
   * @param method HTTP method (GET, POST, PUT, DELETE, etc.)
   * @param url fully qualified URL
   * @param headers HTTP headers from the caller
   * @param body request body (serialized JSON string, raw Buffer, or null)
   * @param options optional per-call flags (e.g. redirect suppression)
   * @returns ApiHttpResponse containing status code, body, and headers
   */
  async sendRequest(
    method: string,
    url: string,
    headers: Record<string, string>,
    body: string | Buffer | Record<string, unknown> | null,
    options?: SendRequestOptions,
  ): Promise<ApiHttpResponse> {
    /* Using a client after close() is a wrong call order: the invalid-state
     * built-in Error, not an SDK error. */
    if (this.closed) {
      throw new Error("ApiClient has been closed and can no longer be used");
    }
    if (body instanceof Blob) {
      body = Buffer.from(await body.arrayBuffer());
    }

    const mergedHeaders: Record<string, string> = {
      ...this.transportOptions.defaultHeaders,
      ...headers,
    };

    if (
      this.transportOptions.userAgent != null &&
      !mergedHeaders["User-Agent"]
    ) {
      mergedHeaders["User-Agent"] = this.transportOptions.userAgent;
    }
    if (
      this.transportOptions.injectRequestId &&
      !mergedHeaders["X-Request-ID"]
    ) {
      mergedHeaders["X-Request-ID"] = globalThis.crypto.randomUUID();
    }

    mergedHeaders["Accept-Encoding"] ??= this.supportedEncodings();

    if (body != null && typeof body === "object" && !Buffer.isBuffer(body)) {
      const boundary = globalThis.crypto.randomUUID();
      mergedHeaders["Content-Type"] =
        `multipart/form-data; boundary=${boundary}`;
      body = await this.buildMultipartBody(
        body as Record<string, unknown>,
        boundary,
      );
    } else if (body == null) {
      delete mergedHeaders["Content-Type"];
      /* Some servers / WAFs treat POST/PUT/PATCH with no body and no
       * Content-Length as malformed (411 Length Required) or behave
       * inconsistently. Set Content-Length: 0 explicitly on body-bearing
       * verbs to match Kotlin's `ByteArray(0)` and the other 11 SDKs. */
      const upperMethod = method.toUpperCase();
      if (
        upperMethod === "POST" ||
        upperMethod === "PUT" ||
        upperMethod === "PATCH"
      ) {
        mergedHeaders["Content-Length"] = "0";
      }
    }

    /* Gap BH: automatic redirect following re-sends Authorization / Cookie /
     * Proxy-Authorization across cross-origin 3xx redirects by default, which
     * leaks bearer tokens to attacker-controlled hosts via malicious 302. We
     * disable automatic redirect following (redirect: 'manual') and follow
     * Location: headers ourselves below, stripping the sensitive headers when
     * the next URL's origin (scheme + host + port) differs from the original. */
    const fetchOptions: Record<string, unknown> = {
      method,
      headers: mergedHeaders,
      body: body ?? undefined,
      redirect: "manual",
    };

    if (this.transportOptions.timeout != null) {
      fetchOptions.signal = AbortSignal.timeout(this.transportOptions.timeout);
    }

    const dispatcher = await this.getDispatcher();
    if (dispatcher) {
      fetchOptions.dispatcher = dispatcher;
    }

    /* A malformed request URL or a header value fetch refuses is a caller
     * mistake. fetch reports both as a TypeError, the same type it uses for
     * a failed connection, so check them here and let the TypeError escape
     * before the transport handler below could mistake it for a
     * NetworkError. */
    new URL(url);
    new Headers(mergedHeaders);

    let response: Response;
    let currentUrl = url;
    const originalUrl = url;
    try {
      response = await fetch(url, fetchOptions as Parameters<typeof fetch>[1]);
      const noRedirect = options?.noRedirect === true;
      if (this.transportOptions.followRedirects && !noRedirect) {
        const maxRedirects = this.transportOptions.maxRedirects ?? 20;
        let hops = 0;
        let currentMethod = method;
        let currentBody: unknown = body;
        let currentHeaders: Record<string, string> = { ...mergedHeaders };
        while (
          hops < maxRedirects &&
          AbstractApiClient.isRedirectStatus(response.status)
        ) {
          const location = response.headers.get("location");
          if (location == null || location.length === 0) break;
          const nextUrl = new URL(location, currentUrl).toString();
          const nextParsed = new URL(nextUrl);
          /* T-D3: a Location pointing at a non-HTTP(S) scheme
           * (file:, javascript:, data:, ...) must be refused loudly,
           * never silently surfaced as the 3xx response. Throw a typed
           * ApiError so callers see the offending URL. */
          if (
            nextParsed.protocol !== "http:" &&
            nextParsed.protocol !== "https:"
          ) {
            throw new ApiError(
              response.status,
              `Refusing to follow redirect to non-HTTP(S) URL: ${nextUrl}`,
              null,
              null,
              null,
            );
          }
          const crossOrigin = !AbstractApiClient.sameOrigin(
            originalUrl,
            nextUrl,
          );

          /* Gap T3: pick follow-up method+body per RFC 7231 §6.4.4 / RFC 7538.
           *   307 + 308: preserve original method and body.
           *   303:       force GET, drop the body (and Content-Type/Length).
           *   301 + 302: historical browser behaviour — switch to GET for
           *              non-GET/HEAD requests, drop the body. */
          const statusCode = response.status;
          let nextMethod: string;
          let nextBody: unknown;
          const methodUpper = currentMethod.toUpperCase();
          if (statusCode === 307 || statusCode === 308) {
            nextMethod = currentMethod;
            nextBody = currentBody;
          } else if (statusCode === 303) {
            nextMethod = "GET";
            nextBody = undefined;
          } else if (methodUpper === "GET" || methodUpper === "HEAD") {
            nextMethod = currentMethod;
            nextBody = currentBody;
          } else {
            nextMethod = "GET";
            nextBody = undefined;
          }

          /* Gap 3.3: refuse to replay a request body across a transport
           * downgrade (HTTPS -> HTTP). Replaying the body in cleartext
           * after the original request was sent over TLS leaks whatever
           * the caller trusted to TLS to protect (credentials, PII,
           * signed tokens). Only 307 / 308 preserve the method AND body
           * across a redirect (RFC 7231 sec 6.4.7 / RFC 7538), so they are
           * the only statuses that can replay a body onto a downgraded
           * http:// target -- restrict the guard to them. 301 / 302 / 303
           * coerce the follow-up to a bodyless GET (handled above), so a
           * downgrade on those statuses carries nothing sensitive and must
           * PROCEED rather than throw; gating only on `nextBody` (without
           * the status check) wrongly refused a GET-with-body whose body
           * the 301/302 coercion preserves. N1: guard 307/308 only. */
          const currentProto = new URL(currentUrl).protocol;
          if (
            (statusCode === 307 || statusCode === 308) &&
            nextBody !== undefined &&
            nextBody !== null &&
            currentProto === "https:" &&
            nextParsed.protocol === "http:"
          ) {
            throw new ApiError(
              response.status,
              "Refusing to replay request body across HTTPS -> HTTP redirect downgrade " +
                `(from ${currentUrl} to ${nextUrl})`,
              null,
              null,
              null,
            );
          }

          const redirectHeaders: Record<string, string> = {};
          for (const [k, v] of Object.entries(currentHeaders)) {
            if (crossOrigin && SENSITIVE_HEADER_NAMES.has(k.toLowerCase()))
              continue;
            if (
              nextBody === undefined &&
              (k.toLowerCase() === "content-type" ||
                k.toLowerCase() === "content-length")
            ) {
              continue;
            }
            redirectHeaders[k] = v;
          }
          const redirectOptions = {
            ...fetchOptions,
            method: nextMethod,
            headers: redirectHeaders,
            body: nextBody,
          };
          currentUrl = nextUrl;
          currentMethod = nextMethod;
          currentBody = nextBody;
          currentHeaders = redirectHeaders;
          response = await fetch(
            nextUrl,
            redirectOptions as Parameters<typeof fetch>[1],
          );
          hops++;
        }
        /* T-D1: exhausting the redirect budget must fail loudly. The
         * other SDKs throw "too many redirects" rather than silently
         * returning the final 3xx as if it were a normal response. */
        if (
          hops >= maxRedirects &&
          AbstractApiClient.isRedirectStatus(response.status)
        ) {
          throw new ApiError(
            response.status,
            `Too many redirects (exceeded maxRedirects=${maxRedirects})`,
            null,
            null,
            null,
          );
        }
      }
    } catch (error) {
      /* ApiError already carries the response's status + message
       * (e.g. the HTTPS -> HTTP downgrade refusal). Propagate it
       * unchanged so callers can match on the original cause. */
      if (error instanceof ApiError) {
        throw error;
      }
      throw AbstractApiClient.transportError(error);
    }

    /* Wrap the post-headers body read in the SDK's transport error type.
     * A connection reset / read-timeout / truncated-chunked failure that
     * occurs AFTER response headers are received surfaces as the same
     * NetworkError (statusCode 0, underlying cause preserved) as a
     * send-phase failure. A body that arrived but cannot be decompressed is
     * a response that could not be used: an ApiError carrying the
     * response's real status. */
    let responseBytes: Buffer;
    try {
      responseBytes = Buffer.from(await response.arrayBuffer());
      /* Gap AL: a server may advertise `Content-Encoding: gzip` (etc.) but
       * send a body that is not a valid stream for that codec (a truncated
       * or non-gzip payload). The fetch layer decodes Content-Encoding
       * transparently and throws on a malformed stream -- that throw is
       * caught above. But when it did NOT decode (the body still carries
       * the codec's signature bytes), passing those raw bytes straight
       * through would silently hand the caller corrupt/undecoded data. Decode
       * the declared encoding ourselves in that case and surface any failure
       * as the SDK's own ApiError, so a Content-Encoding lie can never leak
       * corrupted bytes -- matching java/python/ruby/go/php/rust/elixir which
       * wrap the decompression failure rather than crashing or passing it on. */
      const contentEncoding = (response.headers.get("content-encoding") ?? "")
        .trim()
        .toLowerCase();
      responseBytes = await AbstractApiClient.ensureDecoded(
        responseBytes,
        contentEncoding,
        response.status,
      );
    } catch (error) {
      if (error instanceof ApiError) {
        throw error;
      }
      if (AbstractApiClient.isDecodeFailure(error)) {
        throw new ApiError(
          response.status,
          "failed to decompress response body: " +
            (error instanceof Error ? error.message : String(error)),
          null,
          null,
          null,
          { cause: error },
        );
      }
      throw AbstractApiClient.transportError(error);
    }
    const contentType = response.headers.get("content-type") ?? "";
    const responseBody = AbstractApiClient.isTextContentType(contentType)
      ? AbstractApiClient.decodeBody(responseBytes, contentType)
      : responseBytes.toString("base64");

    /**
     * Gap BE+BF: response header keys are normalised to lowercase so callers
     * can look them up consistently regardless of the casing the server used
     * (HTTP header names are case-insensitive per RFC 7230 section 3.2, and
     * HTTP/2 mandates lowercase on the wire). Repeated header lines (for
     * example multiple Link or Set-Cookie headers) are joined with ", " to
     * preserve order per RFC 7230 section 3.2.2. The fetch Headers object
     * already lowercases keys and joins comma-separable fields. Set-Cookie
     * is exposed via `getSetCookie()`; the joined form is not directly
     * parseable, so callers that need structured cookie access should
     * either parse `set-cookie` themselves or use `response.headers.getSetCookie()`.
     */
    const responseHeaders: Record<string, string> = {};
    const headersWithCookies = response.headers as Headers & {
      getSetCookie?: () => string[];
    };
    response.headers.forEach((value, key) => {
      responseHeaders[key.toLowerCase()] = value;
    });
    if (typeof headersWithCookies.getSetCookie === "function") {
      const cookies = headersWithCookies.getSetCookie();
      if (cookies.length > 0) {
        responseHeaders["set-cookie"] = cookies.join(", ");
      }
    }

    return {
      statusCode: response.status,
      body: responseBody,
      headers: responseHeaders,
    };
  }

  /**
   * Defend against a `Content-Encoding` lie (Gap AL).
   *
   * The fetch layer decodes the declared `Content-Encoding` transparently and
   * throws on a malformed stream, so in the common case `buf` is already the
   * decoded plaintext and the declared codec's signature bytes are gone. When
   * the fetch layer did NOT decode -- the bytes still carry the codec signature
   * (e.g. the gzip magic `0x1F 0x8B`) -- handing them to the caller untouched
   * would silently surface corrupt, undecoded bytes. In that case we decode the
   * gzip stream ourselves with the Web-standard {@link DecompressionStream} (so
   * the guard runs on every runtime) and surface any failure as an
   * {@link ApiError}. Bodies whose declared codec was already decoded (no
   * signature bytes remain) and bodies with no/identity `Content-Encoding` pass
   * through unchanged.
   *
   * @param buf the raw response bytes as read from the body
   * @param contentEncoding the lower-cased `Content-Encoding` header value
   * @param statusCode the status of the response the bytes arrived with
   * @returns the decoded bytes (or `buf` unchanged when no decode is needed)
   * @throws ApiError carrying `statusCode` when the declared encoding cannot
   *   be decoded
   */
  static async ensureDecoded(
    buf: Buffer,
    contentEncoding: string,
    statusCode = 0,
  ): Promise<Buffer> {
    if (contentEncoding === "" || contentEncoding === "identity") {
      return buf;
    }
    /* Only the first declared coding can still be on the wire; a comma list
     * (e.g. "gzip, br") is decoded outer-first by the fetch layer, so inspect
     * the last (outermost) token that remains. We only act when the bytes still
     * carry a codec signature, which means the fetch layer left them encoded. */
    if (contentEncoding === "gzip" || contentEncoding === "x-gzip") {
      /* gzip magic: 0x1F 0x8B. Absent => already decoded by the fetch layer => leave. */
      if (buf.length >= 2 && buf[0] === 0x1f && buf[1] === 0x8b) {
        try {
          const stream = new Blob([buf])
            .stream()
            .pipeThrough(new DecompressionStream("gzip"));
          const decoded = await new Response(stream).arrayBuffer();
          return Buffer.from(decoded);
        } catch (error) {
          throw new ApiError(
            statusCode,
            `failed to decompress response body (content-encoding=${contentEncoding}): ` +
              (error instanceof Error ? error.message : String(error)),
            null,
            null,
            null,
            { cause: error },
          );
        }
      }
    }
    return buf;
  }

  /**
   * Build a multipart/form-data request body from a map of form fields.
   *
   * @param formParts the form field names and values
   * @param boundary the multipart boundary string
   * @returns the assembled multipart body as a Buffer
   */
  private async buildMultipartBody(
    formParts: Record<string, unknown>,
    boundary: string,
  ): Promise<Buffer> {
    const parts: Buffer[] = [];
    for (const [name, value] of Object.entries(formParts)) {
      if (Array.isArray(value)) {
        for (const item of value) {
          parts.push(await this.multipartPart(name, item, boundary));
        }
      } else {
        parts.push(await this.multipartPart(name, value, boundary));
      }
    }
    parts.push(Buffer.from(`--${boundary}--\r\n`, "utf-8"));
    return Buffer.concat(parts);
  }

  /**
   * Build a single multipart part.
   *
   * @param name the field name
   * @param value the field value
   * @param boundary the multipart boundary string
   * @returns the assembled part as a Buffer
   */
  private async multipartPart(
    name: string,
    value: unknown,
    boundary: string,
  ): Promise<Buffer> {
    if (Buffer.isBuffer(value)) {
      const disposition = AbstractApiClient.buildContentDisposition(name, name);
      const mimeType = AbstractApiClient.mimeTypeForFilename(name);
      const header = `--${boundary}\r\nContent-Disposition: ${disposition}\r\nContent-Type: ${mimeType}\r\n\r\n`;
      return Buffer.concat([
        Buffer.from(header, "utf-8"),
        value,
        Buffer.from("\r\n", "utf-8"),
      ]);
    }
    if (value instanceof Blob) {
      const buf = Buffer.from(await value.arrayBuffer());
      const filename = (value as Blob & { name?: string }).name ?? name;
      const disposition = AbstractApiClient.buildContentDisposition(
        name,
        filename,
      );
      const mimeType =
        value.type && value.type.length > 0
          ? value.type
          : AbstractApiClient.mimeTypeForFilename(filename);
      const header = `--${boundary}\r\nContent-Disposition: ${disposition}\r\nContent-Type: ${mimeType}\r\n\r\n`;
      return Buffer.concat([
        Buffer.from(header, "utf-8"),
        buf,
        Buffer.from("\r\n", "utf-8"),
      ]);
    }
    if (typeof value === "object" && value !== null) {
      AbstractApiClient.assertNoControlChars(name, "multipart field name");
      const escapedName = name.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
      /*
       * Serialize the JSON part through ObjectSerializer rather than raw
       * JSON.stringify so the custom replacer (Date -> local-offset
       * date-time, Buffer -> base64, Temporal -> protobuf-JSON,
       * null-stripping) is applied — matching the JSON body path in
       * base-api and the other 11 SDKs. Raw JSON.stringify would emit
       * Date.toJSON() (UTC `...Z`), `{ type: 'Buffer', data: [...] }` for
       * byte fields and the polyfill's internal object for Temporal types,
       * all corrupt on the wire.
       */
      const json = ObjectSerializer.serialize(value);
      return Buffer.from(
        `--${boundary}\r\nContent-Disposition: form-data; name="${escapedName}"\r\nContent-Type: application/json\r\n\r\n${json}\r\n`,
        "utf-8",
      );
    }
    AbstractApiClient.assertNoControlChars(name, "multipart field name");
    const escapedName = name.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
    return Buffer.from(
      `--${boundary}\r\nContent-Disposition: form-data; name="${escapedName}"\r\n\r\n${String(value)}\r\n`,
      "utf-8",
    );
  }

  /**
   * Decode response bytes using the charset declared in the
   * `Content-Type` header. Defaults to UTF-8 when no charset is
   * present, and falls back to UTF-8 if the declared charset is not
   * recognised by the runtime's `TextDecoder`.
   *
   * A bare `utf-16`/`utf16` label (no explicit byte order) is decoded as
   * big-endian. Node's `TextDecoder` maps the WHATWG `'utf-16'` label to
   * little-endian, but for an HTTP client resolving the IANA/MIME charset
   * `UTF-16`, RFC 2781 specifies big-endian as the default when no BOM is
   * present. A leading BOM is still honoured by `TextDecoder` and overrides
   * the byte order; explicit `utf-16le`/`utf-16be` labels are left untouched.
   *
   * @param buf the raw response bytes
   * @param contentType the response Content-Type header value
   * @returns the decoded text
   */
  static decodeBody(buf: Buffer, contentType?: string): string {
    const m = /charset=([^;]+)/i.exec(contentType ?? "");
    let cs = m
      ? m[1]
          .trim()
          .toLowerCase()
          .replace(/^["']|["']$/g, "")
      : "utf-8";
    if (cs === "utf-16" || cs === "utf16") {
      cs = "utf-16be";
    }
    try {
      return new TextDecoder(cs, { fatal: false }).decode(buf);
    } catch {
      return buf.toString("utf-8");
    }
  }

  /**
   * Map common file extensions to their canonical MIME types for use
   * as the per-part `Content-Type` in multipart uploads.
   * Falls back to `application/octet-stream` when the extension
   * is unknown.
   */
  private static readonly EXTENSION_MIME_TYPES: Record<string, string> = {
    png: "image/png",
    jpg: "image/jpeg",
    jpeg: "image/jpeg",
    gif: "image/gif",
    webp: "image/webp",
    bmp: "image/bmp",
    svg: "image/svg+xml",
    tif: "image/tiff",
    tiff: "image/tiff",
    ico: "image/x-icon",
    pdf: "application/pdf",
    json: "application/json",
    xml: "application/xml",
    zip: "application/zip",
    gz: "application/gzip",
    tar: "application/x-tar",
    txt: "text/plain",
    csv: "text/csv",
    html: "text/html",
    htm: "text/html",
    css: "text/css",
    js: "application/javascript",
    mp3: "audio/mpeg",
    wav: "audio/wav",
    mp4: "video/mp4",
    webm: "video/webm",
    mov: "video/quicktime",
    doc: "application/msword",
    docx: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    xls: "application/vnd.ms-excel",
    xlsx: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
  };

  /**
   * Look up the MIME type for the given filename based on its extension.
   *
   * @param filename the filename to inspect
   * @returns the matching MIME type, or `application/octet-stream`
   */
  static mimeTypeForFilename(filename: string): string {
    const dot = filename.lastIndexOf(".");
    if (dot < 0 || dot === filename.length - 1) {
      return "application/octet-stream";
    }
    const ext = filename.substring(dot + 1).toLowerCase();
    return (
      AbstractApiClient.EXTENSION_MIME_TYPES[ext] ?? "application/octet-stream"
    );
  }

  /**
   * Build a `Content-Disposition` header value for a multipart
   * file part, sanitising the supplied filename to prevent CRLF header
   * injection and ensuring non-ASCII filenames are transmitted via the
   * RFC 5987 `filename*` parameter alongside an ASCII fallback.
   *
   * @param name the form field name
   * @param filename the file name to advertise
   * @returns the assembled `Content-Disposition` value
   * @throws TypeError when the filename contains control characters
   */
  static buildContentDisposition(name: string, filename: string): string {
    AbstractApiClient.assertNoControlChars(filename, "multipart filename");
    AbstractApiClient.assertNoControlChars(name, "multipart field name");
    const escapedName = name.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
    let isAscii = true;
    let asciiFallback = "";
    for (const ch of filename) {
      const code = ch.codePointAt(0)!;
      if (code >= 0x20 && code <= 0x7e) {
        asciiFallback += ch;
      } else {
        isAscii = false;
        asciiFallback += "_";
      }
    }
    const escapedFilename = asciiFallback
      .replace(/\\/g, "\\\\")
      .replace(/"/g, '\\"');
    let header = `form-data; name="${escapedName}"; filename="${escapedFilename}"`;
    if (!isAscii) {
      header += `; filename*=UTF-8''${encodeURIComponent(filename)}`;
    }
    return header;
  }

  /**
   * Reject strings containing CR, LF, or NUL bytes which could otherwise
   * split the `Content-Disposition` header and inject arbitrary
   * additional headers or a forged body.
   *
   * @param value the candidate string
   * @param label human-readable label included in the error message
   */
  private static assertNoControlChars(value: string, label: string): void {
    for (let i = 0; i < value.length; i++) {
      const code = value.charCodeAt(i);
      if (code === 0x0a || code === 0x0d || code === 0x00) {
        throw new TypeError(`Invalid ${label}: contains CR, LF, or NUL`);
      }
    }
  }

  /**
   * Determines whether the given content type represents text content
   * that is safe to decode as a UTF-8 string.
   *
   * @param contentType the Content-Type header value
   * @returns true for text-like content types, false otherwise
   */
  private static isTextContentType(contentType: string): boolean {
    const mediaType = contentType.split(";")[0].trim().toLowerCase();
    if (!mediaType) {
      return true;
    }
    if (mediaType.startsWith("text/")) {
      return true;
    }
    return (
      mediaType === "application/json" ||
      mediaType === "application/xml" ||
      mediaType === "application/javascript" ||
      mediaType.endsWith("+json") ||
      mediaType.endsWith("+xml")
    );
  }

  /**
   * Return true for HTTP 3xx status codes that carry a Location header.
   */
  private static isRedirectStatus(code: number): boolean {
    return (
      code === 301 ||
      code === 302 ||
      code === 303 ||
      code === 307 ||
      code === 308
    );
  }

  /**
   * Compare two URLs by scheme + host + effective port.
   */
  private static sameOrigin(a: string, b: string): boolean {
    try {
      const ua = new URL(a);
      const ub = new URL(b);
      if (ua.protocol !== ub.protocol) return false;
      if (ua.hostname.toLowerCase() !== ub.hostname.toLowerCase()) return false;
      const portA =
        ua.port !== "" ? ua.port : ua.protocol === "https:" ? "443" : "80";
      const portB =
        ub.port !== "" ? ub.port : ub.protocol === "https:" ? "443" : "80";
      return portA === portB;
    } catch {
      return false;
    }
  }

  /**
   * Closes the client. After calling this method the client must not be
   * reused; a subsequent {@link sendRequest} raises an {@link ApiError}.
   * Subclasses override to release transport resources (e.g. an undici
   * dispatcher pool).
   */
  async close(): Promise<void> {
    this.closed = true;
    return Promise.resolve();
  }
}
